...
class HierarchicalReasoningModel_ACTV1Block(nn.Module):
    ...
    def forward(self, cos_sin: CosSin, hidden_states: torch.Tensor, value_embed) -> torch.Tensor:
        ...
        hidden_states = rms_norm(
            hidden_states + self.self_attn(cos_sin=cos_sin, hidden_states=hidden_states, value_embed=value_embed),
...
class HierarchicalReasoningModel_ACTV1ReasoningModule(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_injection: torch.Tensor, value_embed,
                **kwargs) -> torch.Tensor:
        # Input injection (add)
...
class HierarchicalReasoningModel_ACTV1_Inner(nn.Module):
    def __init__(self, config: HierarchicalReasoningModel_ACTV1Config) -> None:
        ...
        self.value_embed_tokens = CastedEmbedding(self.config.vocab_size, self.config.hidden_size,
        ...
        if self.config.puzzle_emb_ndim > 0:
            ...
            self.puzzle_emb = CastedSparseEmbedding(self.config.num_puzzle_identifiers, self.config.puzzle_emb_ndim,
                                                    ...
                                                    cast_to=self.forward_dtype)
            self.value_puzzle_emb = CastedSparseEmbedding(self.config.num_puzzle_identifiers,
